// description: シンガポールの人口密度がフィリピンよりも高いことを確認する。
// file_path: src/lib/skills/populationDensity/SingaporeHigherThanPhilippines.ts
...
/**
 * Checks if Singapore's population density is higher than Philippines's.
 * @returns Promise resolving to a boolean indicating whether Singapore's population density is higher.
 */
const isPopulationDensityOfSingaporeHigherThanPhilippines =
  async (): Promise<boolean> => {
    ...
    // Fetch Philippines's geojson data
    const philippinesOverpassQuery = `[out:json];relation["name:en"="Philippines"]["admin_level"=2];out geom;`;
    const philippinesOverpassData = await fetchOverpassData(philippinesOverpassQuery);
    const philippinesGeojsonData = osmtogeojson(philippinesOverpassData);
    // Fetch Philippines's population
    const philippinesPopulationData = await fetchWorldBankTotalPopulation("ph");
    const philippinesPopulation = philippinesPopulationData[1][0].value;
    // Calculate Philippines's population density
    const philippinesPopulationDensity = calculatePopulationDensity(
      philippinesGeojsonData,
      philippinesPopulation
    );
    // Compare the population densities
    return singaporePopulationDensity > philippinesPopulationDensity;
  };

export default isPopulationDensityOfSingaporeHigherThanPhilippines;
