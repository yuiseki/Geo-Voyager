...
/**
 * Checks if Singapore's population density is higher than the Philippines'.
...
out geom;`;
      ...
      // Fetch Philippines's area and population
      const queryPhilippines = `[out:json];
relation["name:en"="Philippines"]["admin_level"=2];
out geom;`;
      const dataPhilippines = await fetchOverpassData(queryPhilippines);
      const geoJsonPhilippines = osmtogeojson(dataPhilippines);
      const areaPhilippines = turf.area(geoJsonPhilippines);
      const populationPhilippines = await fetchWorldBankPopulation("PH");
      ...
      const densityPhilippines = calculatePopulationDensity(
        areaPhilippines,
        populationPhilippines
      );

      return densitySingapore > densityPhilippines;
    } catch (error) {
