...
/**
 * Checks if Myanmar's population density is higher than Singapore's.
 * @returns Promise resolving to a boolean indicating whether Myanmar's population density is higher.
 */
const isPopulationDensityOfMyanmarHigherThanSingapore =
  async (): Promise<boolean> => {
    // Fetch Myanmar's GeoJSON data
    const myanmarOverpassQuery = `[out:json];relation["name:en"="Myanmar"]["admin_level"=2];out geom;`;
    const myanmarOverpassData = await fetchOverpassData(myanmarOverpassQuery);
    const myanmarGeojsonData = osmtogeojson(myanmarOverpassData);
    // Fetch Myanmar's population data
    const myanmarPopulationData = await fetchWorldBankTotalPopulation("mm");
    const myanmarPopulation = myanmarPopulationData[1][0].value;
    // Calculate Myanmar's population density
    const myanmarPopulationDensity = calculatePopulationDensity(
      myanmarGeojsonData,
      myanmarPopulation
    );
    ...
    return myanmarPopulationDensity > singaporePopulationDensity;
  };

export default isPopulationDensityOfMyanmarHigherThanSingapore;
