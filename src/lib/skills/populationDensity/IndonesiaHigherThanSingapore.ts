 ...
 * @return boolean
 */
const isPopulationDensityOfIndonesiaHigherThanSingapore =
  async (): Promise<boolean> => {
    // Fetch Indonesia's geojson data
    const indonesiaOverpassQuery = `[out:json];["name"="Indonesia"]["admin_level"=2];out geom;`;
    const indonesiaOverpassData = await fetchOverpassData(
      indonesiaOverpassQuery
    );
    const indonesiaGeojsonData = osmtogeojson(indonesiaOverpassData);
    // Fetch Indonesia's population data
