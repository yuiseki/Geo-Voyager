...
const isPopulationDensityOfSingaporeHigherThanIndonesia =
  async (): Promise<boolean> => {
    ...
    // Fetch Indonesia's geojson data
    const indonesiaOverpassQuery = `[out:json];relation["name"="Indonesia"]["admin_level"=2];out geom;`;
    const indonesiaOverpassData = await fetchOverpassData(indonesiaOverpassQuery);
    const indonesiaGeojsonData = osmtogeojson(indonesiaOverpassData);
    // Fetch Indonesia's population
    const indonesiaPopulationData = await fetchWorldBankTotalPopulation("id");
    const indonesiaPopulation = indonesiaPopulationData[1][0].value;
    // Calculate Indonesia's population density
    const indonesiaPopulationDensity = calculatePopulationDensity(
      indonesiaGeojsonData,
      indonesiaPopulation
    );
    // Compare the population densities
    return singaporePopulationDensity > indonesiaPopulationDensity;
  };
