// description: シンガポールの人口密度がモザンビークよりも高いことを確認する。
// file_path: src/lib/skills/populationDensity/SingaporeHigherThanMozambique.ts
...
/**
 * Checks if Singapore's population density is higher than Mozambique's.
 * @returns Promise resolving to a boolean indicating whether Singapore's population density is higher.
 */
const isPopulationDensityOfSingaporeHigherThanMozambique =
  async (): Promise<boolean> => {
    ...
    // Fetch Mozambique's geojson data
    const mozambiqueOverpassQuery = `[out:json];relation["name:en"="Mozambique"]["admin_level"=2];out geom;`;
    const mozambiqueOverpassData = await fetchOverpassData(mozambiqueOverpassQuery);
    const mozambiqueGeojsonData = osmtogeojson(mozambiqueOverpassData);
    // Fetch Mozambique's population
    const mozambiquePopulationData = await fetchWorldBankTotalPopulation("mz");
    const mozambiquePopulation = mozambiquePopulationData[1][0].value;
    // Calculate Mozambique's population density
    const mozambiquePopulationDensity = calculatePopulationDensity(
      mozambiqueGeojsonData,
      mozambiquePopulation
    );
    // Compare the population densities
    return singaporePopulationDensity > mozambiquePopulationDensity;
  };

export default isPopulationDensityOfSingaporeHigherThanMozambique;