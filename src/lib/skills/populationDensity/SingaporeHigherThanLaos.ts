...
/**
 * Checks if Singapore's population density is higher than Laos's.
...
const isPopulationDensityOfSingaporeHigherThanLaos =
  async (): Promise<boolean> => {
    ...
    const laosOverpassQuery = `[out:json];relation["name:en"="Laos"]["admin_level"=2];out geom;`;
    const laosOverpassData = await fetchOverpassData(laosOverpassQuery);
    const laosGeojsonData = osmtogeojson(laosOverpassData);
    // Fetch Laos's population
    const laosPopulationData = await fetchWorldBankTotalPopulation("la");
    ...
    return singaporePopulationDensity > laosPopulationDensity;
  };
