// description: 東京都において、人口あたりの公園の数が最も高い行政区が稲城市であることを確認する。
// file_path: src/lib/skills/admins/Japan/Tokyo/checkInagiIsMostParksPerPopulationWardInTokyo.ts
...
const getMostParksPerPopulationAdminInTokyo = async (): Promise<string> => {
  ...
  for (const adminArea of adminNames.split("\n")) {
...
const checkInagiIsMostParksPerPopulationWardInTokyo =
  async (): Promise<boolean> => {
    ...
    return mostParksPerPopulationAdmin.includes("稲城市");
  };

export default checkInagiIsMostParksPerPopulationWardInTokyo;
