// description: 東京都において、学校の数が最も多い行政区が世田谷区であることを確認する。
// file_path: src/lib/skills/admins/Japan/Tokyo/checkSetagayaIsMostSchoolsWardInTokyo.ts
...
const checkSetagayaIsMostSchoolsWardInTokyo = async (): Promise<boolean> => {
  const wardWithMostSchools = await getAdminWithMostSchoolsInTokyo();
  return wardWithMostSchools.includes("世田谷区");
};

export default checkSetagayaIsMostSchoolsWardInTokyo;