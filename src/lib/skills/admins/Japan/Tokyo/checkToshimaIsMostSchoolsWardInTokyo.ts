// description: 東京都において、学校の数が最も多い行政区が板橋区であることを確認する。
// file_path: src/lib/skills/admins/Japan/Tokyo/checkToshimaIsMostSchoolsWardInTokyo.ts
...
const checkToshimaIsMostSchoolsWardInTokyo = async (): Promise<boolean> => {
  const wardWithMostSchools = await getAdminWithMostSchoolsInTokyo();
  return wardWithMostSchools.includes("豊島区");
};

export default checkToshimaIsMostSchoolsWardInTokyo;