// description: 東京都において、学校の数が最も多い行政区が杉並区であることを確認する。
// file_path: src/lib/skills/admins/Japan/Tokyo/checkOtaIsMostSchoolsWardInTokyo.ts
...
const checkOtaIsMostSchoolsWardInTokyo = async (): Promise<boolean> => {
  const wardWithMostSchools = await getAdminWithMostSchoolsInTokyo();
  return wardWithMostSchools.includes("大田区");
};

export default checkOtaIsMostSchoolsWardInTokyo;