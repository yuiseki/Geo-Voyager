// description: 東京都において、学校の数が最も多い行政区が江戸川区であることを確認する。
// file_path: src/lib/skills/admins/Japan/Tokyo/checkEdogawaIsMostSchoolsWardInTokyo.ts
...
const checkEdogawaIsMostSchoolsWardInTokyo = async (): Promise<boolean> => {
  const wardWithMostSchools = await getAdminWithMostSchoolsInTokyo();
  return wardWithMostSchools.includes("江戸川区");
};

export default checkEdogawaIsMostSchoolsWardInTokyo;