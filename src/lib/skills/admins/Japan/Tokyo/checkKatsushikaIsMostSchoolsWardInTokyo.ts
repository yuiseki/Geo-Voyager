// description: 東京都において、学校の数が最も多い行政区が葛飾区であることを確認する。
// file_path: src/lib/skills/admins/Japan/Tokyo/checkKatsushikaIsMostSchoolsWardInTokyo.ts
...
const checkKatsushikaIsMostSchoolsWardInTokyo = async (): Promise<boolean> => {
  const wardWithMostSchools = await getAdminWithMostSchoolsInTokyo();
  return wardWithMostSchools.includes("葛飾区");
};

export default checkKatsushikaIsMostSchoolsWardInTokyo;