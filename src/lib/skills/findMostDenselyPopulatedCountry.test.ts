...
describe('findMostDenselyPopulatedCountry', () => {
  it('should find the most densely populated country in the world', async () => {
    ...
    // 人口密度データを取得
    const populationDensity = await getWorldBankPopulationDensityByCountryCode(countryCode);
    
    // 人口密度が高いことを検証
    expect(populationDensity).toBeGreaterThan(1000); // 1平方km当たり1000人以上が期待される
    
    // 結論のみを簡潔に出力 (詳細データを含む)
    console.log(`世界で最も人口密度が高い国は${result}で、人口密度は${populationDensity.toFixed(2)}人/km²です`);
  }, 60000); // タイムアウトを60秒に設定
});