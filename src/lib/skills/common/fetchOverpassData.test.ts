...
describe('fetchOverpassData', () => {
  ...
  it('should return cached data if available', async () => {
    ...
    expect(fs.promises.readFile).toHaveBeenCalledWith('./tmp/cache/overpass/query_mockedHash.json', 'utf-8');
  ...
  it('should fetch data from API if cache is not available', async () => {
    ...
    expect(fs.promises.readFile).toHaveBeenCalledWith('./tmp/cache/overpass/query_mockedHash.json', 'utf-8');
    expect(console.debug).toHaveBeenCalledWith('Cache not found. Calling Overpass API...');
    expect(fetch).toHaveBeenCalledWith('https://overpass-api.de/api/interpreter', {
      method: 'POST',
      headers: {
    ...
    expect(fs.promises.mkdir).toHaveBeenCalledWith('./tmp/cache/overpass', { recursive: true });
  ...
  it('should throw an error if the API response is not ok', async () => {
    ...
    await expect(fetchOverpassData('test query')).rejects.toThrow('HTTP error! status: 500');
  });
});