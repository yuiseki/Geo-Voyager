...
// file_path: src/lib/skills/checkMacauIsMostDenselyPopulatedCountryInWorld.ts
...
const fetchWorldBankPopulationDensity = async (
  ...
  const rows = data[1];
  const value = rows.filter((row: any) => row.value !== null && row.value !== 0)[0]
    .value;
  return value;
};
 ...
 */
const getCountryNameByAlpha2Codes = async (alpha2Code: string): Promise<string> => {
  let data;
...
const checkMacauIsMostDenselyPopulatedCountryInWorld = async (): Promise<boolean> => {
  ...
  return mostDenselyPopulatedCountry.includes("Macau");
};

export default checkMacauIsMostDenselyPopulatedCountryInWorld;